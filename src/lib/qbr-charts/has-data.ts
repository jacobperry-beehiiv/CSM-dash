 ...
 * UI greys out these tiles and the deck-builder defaults to leaving
 * them out.
 *
 ...
 * gets flagged as no-data and excluded from the deck by default.
 * That's fine — if it matters, the CSM can manually re-include it
 * from the tile.
 */
...
export function specHasData(spec: ChartSpec): boolean {
  ...
  // For non-scalars: sum |value| across every series cell.
  let total = 0;
